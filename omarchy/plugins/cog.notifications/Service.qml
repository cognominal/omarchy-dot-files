...
Item {
  ...
  function invokePopupDefault(index) {
    ...
    // Always focus the sender too: apps that invoke "default" ask to raise
    // themselves via xdg-activation, which Hyprland ignores for windows with
    // focus_on_activate = false (e.g. Telegram). Focusing directly from the
    // shell bypasses that, while still letting the app react to the click.
    focusApp(entry)
    dismissPopup(index)
  }
  ...
  // appIcon is usually the desktop id (org.telegram.desktop), which matches
  // the window class where the display name ("Telegram Desktop") does not.
  function focusApp(entry) {
    if (!entry) return
    var helper = service.omarchyPath + "/bin/omarchy-hyprland-focus-app"
    var names = []
    if (entry.appIcon && String(entry.appIcon).indexOf("/") < 0) names.push(String(entry.appIcon))
    if (entry.app) {
      var app = String(entry.app)
      names.push(app)
      // "Telegram Desktop" -> "Telegram", which matches org.telegram.desktop.
      var first = app.split(/\s+/)[0]
      if (first.length >= 3 && first !== app) names.push(first)
    }
    if (names.length === 0) return
    focusAppProc.command = ["bash", "-c", 'for n in "${@:2}"; do "$1" "$n" && exit 0; done; exit 1', "_", helper].concat(names)
